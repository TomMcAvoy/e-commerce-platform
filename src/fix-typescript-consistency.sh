#!/bin/bash
# filepath: ./fix-typescript-consistency.sh
...
#
# It resolves two main issues:
#   1. Inconsistent casing in 'AppError' imports (e.g., 'appError' vs. 'AppError').
#   2. Incorrect path for the 'errorHandler' middleware import in 'src/index.ts'.
# ==============================================================================
...
find "$SRC_DIR" -type f -name "*.ts" | while read -r file; do
  ...
  # Correct both relative paths ('../' and './')
  if grep -q "from '..\/utils\/appError'" "$file" || grep -q "from '.\/utils\/appError'" "$file"; then
...
echo "--------------------------------------------------------------------"
echo "The compilation errors should be gone. Please run the tests again."
echo ""
echo "   npm test"
echo ""
